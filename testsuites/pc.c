...
static int pcContains(fer_pc_t *pc, const fer_vec2_t *v)
{
    fer_pc_it_t it;
    const fer_vec_t *w;
    ...
    while (!ferPCItEnd(&it)){
        w = ferPCItGet(&it);
        if (ferVec2Eq((const fer_vec2_t *)w, v))
            return 1;
...
static void pcPrint(fer_pc_t *pc)
{
...
TEST(ppcPermutate)
{
    ...
    for (i = 0; i < 10; i++){
        ferVec2Add2(&w, (fer_vec2_t *)ferPCGet(pc, 0), (fer_vec2_t *)ferPCGet(pc, 1));
    }
