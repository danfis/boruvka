                 ...
                 void *data)
{
    ...
    // and contact points
    for (i = 0; i < con->num; i++){
        fprintf(stderr, "    [%02d] depth: %f, pos: %f %f %f, dir: %f %f %f\n",
                (int)i, con->depth[i],
                ferVec3X(&con->pos[i]), ferVec3Y(&con->pos[i]),
                ferVec3Z(&con->pos[i]),
                ferVec3X(&con->dir[i]), ferVec3Y(&con->dir[i]),
                ferVec3Z(&con->dir[i]));
    }
...
int main(int argc, char *argv[])
{
    ...
    // Increase maximal number of contacts per pair to 100
    params.max_contacts = 100;
