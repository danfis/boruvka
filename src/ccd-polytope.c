...
_fer_inline void _ferPtNearestUpdate(fer_pt_t *pt, fer_pt_el_t *el)
{
...
static void _ferPtNearestRenew(fer_pt_t *pt)
{
    fer_pt_vertex_t *v;
    fer_pt_edge_t *e;
    fer_pt_face_t *f;
    ...
    FER_LIST_FOR_EACH_ENTRY(&pt->vertices, fer_pt_vertex_t, v, list){
        _ferPtNearestUpdate(pt, (fer_pt_el_t *)v);
    }

    FER_LIST_FOR_EACH_ENTRY(&pt->edges, fer_pt_edge_t, e, list){
        _ferPtNearestUpdate(pt, (fer_pt_el_t *)e);
    }

    FER_LIST_FOR_EACH_ENTRY(&pt->faces, fer_pt_face_t, f, list){
        _ferPtNearestUpdate(pt, (fer_pt_el_t *)f);
    }
}
...
void ferPtDestroy(fer_pt_t *pt)
{
    fer_pt_face_t *f, *f2;
    fer_pt_edge_t *e, *e2;
    fer_pt_vertex_t *v, *v2;

    // first delete all faces
    FER_LIST_FOR_EACH_ENTRY_SAFE(&pt->faces, fer_pt_face_t, f, fer_pt_face_t, f2, list){
        ferPtDelFace(pt, f);
    }

    // delete all edges
    FER_LIST_FOR_EACH_ENTRY_SAFE(&pt->edges, fer_pt_edge_t, e, fer_pt_edge_t, e2, list){
        ferPtDelEdge(pt, e);
    }

    // delete all vertices
    FER_LIST_FOR_EACH_ENTRY_SAFE(&pt->vertices, fer_pt_vertex_t, v, fer_pt_vertex_t, v2, list){
        ferPtDelVertex(pt, v);
    }
}
...
{
    fer_pt_vertex_t *vert;

    vert = FER_ALLOC(fer_pt_vertex_t);
    vert->type = FER_PT_VERTEX;
    ferSupportCopy(&vert->v, v);
    ...
    _ferPtNearestUpdate(pt, (fer_pt_el_t *)vert);
                                          ...
                                          fer_pt_vertex_t *v2)
{
    ...
    edge = FER_ALLOC(fer_pt_edge_t);
    edge->type = FER_PT_EDGE;
    edge->vertex[0] = v1;
    edge->vertex[1] = v2;
    ...
    // update position in .nearest array
    _ferPtNearestUpdate(pt, (fer_pt_el_t *)edge);
                                          ...
                                          fer_pt_edge_t *e3)
{
    ...
    face = FER_ALLOC(fer_pt_face_t);
    face->type = FER_PT_FACE;
    face->edge[0] = e1;
    face->edge[1] = e2;
    face->edge[2] = e3;
    ...
    // update position in .nearest array
    _ferPtNearestUpdate(pt, (fer_pt_el_t *)face);
...
void ferPtDumpSVT(fer_pt_t *pt, const char *fn)
{
    ...
    ferPtDumpSVT2(pt, fout);

    fclose(fout);
}

void ferPtDumpSVT2(fer_pt_t *pt, FILE *fout)
{
    fer_pt_vertex_t *v, *a, *b, *c;
    fer_pt_edge_t *e;
    fer_pt_face_t *f;
    size_t i;
