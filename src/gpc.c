...
/** Create randomly generated initial population in gpc->pop[0] */
static void createInitPop(fer_gpc_t *gpc)
{
    size_t i, len;

    len = gpc->params.pop_size;
    for (i = 0; i < len; i++){
        gpc->pop[0][i] = ferGPCTreeNew();
        gpc->pop[0][i]->root = genRandTree(gpc, 0, gpc->params.max_depth);
        gpc->pop[1][i] = NULL;
...
static void evalPop(fer_gpc_t *gpc, fer_gpc_tree_t **pop)
{
    size_t i;
    ...
    for (i = 0; i < gpc->params.pop_size; i++){
        overall_fitness += evalTree(gpc, pop[i]);
    }
